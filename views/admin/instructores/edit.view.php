...
<main role="main" class="container py-5" style="margin-top: 56px">
    ...
    <form class="row mt-4" method="POST" action="/admin/instructores">
        ...
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control <?= isValidInput($errors, "nombre") ?>" id="nombre" name="nombre" value="<?= cleanOld("nombre", $instructor["nombre"]) ?>">
        ...
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control <?= isValidInput($errors, "apellido") ?>" id="apellido" name="apellido" value="<?= cleanOld("apellido", $instructor["apellido"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['apellido'] ?>
            </div>
        </div>
        <div class="col-12 mb-3">
            ...
            <input type="text" class="form-control <?= isValidInput($errors, "username") ?>" id="username" name="username" value="<?= cleanOld("username", $instructor["username"]) ?>">
        ...
        <div class="col-12 mb-3">
            ...
            <input type="email" class="form-control <?= isValidInput($errors, "email") ?>" id="email" name="email" value="<?= cleanOld("email", $instructor["email"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['email'] ?>
            </div>
        </div>
        <div class="col-md-12 mb-3">
            ...
            <select class="form-select <?= isValidInput($errors, "genero") ?>" id="genero" name="genero">
                <option value="0" <?= cleanOld("genero", $instructor["genero"]) === "0" ? "selected" : "" ?>>Masculino</option>
                <option value="1" <?= cleanOld("genero", $instructor["genero"]) === "1" ? "selected" : "" ?>>Femenino</option>
            </select>
            <div class="invalid-feedback">
                <?= $errors['genero'] ?>
            </div>
        ...
        <div class="col-md-12 mb-3">
            ...
            <select class="form-select <?= isValidInput($errors, "estudios") ?>" id="estudios" name="estudios">
                <?php foreach ($nivelEstudios as $key => $value) : ?>
                    <option value="<?= $key ?>" <?= cleanOld("estudios", $instructor["estudios"]) === $key ? "selected" : "" ?>><?= $value ?></option>
                <?php endforeach; ?>
            </select>
            <div class="invalid-feedback">
                <?= $errors['estudios'] ?>
