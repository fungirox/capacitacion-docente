...
<script>
    $(document).ready(() => {
        ...
        const previousInstructorId = '<?= cleanOld("instructor", $curso["instructor_id"]) ?>';
...
<main role="main" class="container pt-5" style="margin-top: 56px">
    ...
    <form class="row py-4" method="POST" action="/admin/cursos">
        ...
        <div class="col-12 mb-3">
            ...
            <input type="text" class="form-control <?= isValidInput($errors, "nombre") ?>" id="nombre" name="nombre" value="<?= cleanOld("nombre", $curso["nombre"]) ?>">
        ...
        <div class="col-12 mb-3">
            ...
            <textarea class="form-control <?= isValidInput($errors, "descripcion") ?>" rows="3" id="descripcion" name="descripcion"><?= cleanOld("descripcion", $curso["descripcion"]) ?></textarea>
            <div class="invalid-feedback">
                <?= $errors['descripcion'] ?>
            </div>
        </div>
        ...
        <div class="col-12 mb-3">
            ...
            <?php foreach ($areas as $area): ?>
                <div class="form-check">
                    <input class="form-check-input" <?= in_array($area["id"], old("areas", $curso["areas"])) ? "checked" : "" ?> type="checkbox" id="area-<?= $area["id"] ?>" name="areas[]" value="<?= $area["id"] ?>">
        ...
        <div class="col-12">
            ...
            <select name="perfil" id="perfil" class="form-select <?= isValidInput($errors, "perfil") ?>" aria-label="Perfil">
                <option value="0" <?= cleanOld("perfil", $curso["perfil"]) === "0" ? "selected" : "" ?>>Actualización profesional</option>
                <option value="1" <?= cleanOld("perfil", $curso["perfil"]) === "1" ? "selected" : "" ?>>Formación docente</option>
            </select>
            <div class="invalid-feedback">
                <?= $errors['perfil'] ?>
            </div>
        ...
        <div class="col-12 d-flex justify-content-center mb-3">
            <div class="col-12 col-md-8 col-lg-6 btn-group" name="modalidad" role="group">
                <input type="radio" class="btn-check" name="modalidad" id="modalidad-presencial" value="presencial" autocomplete="off" <?= cleanOld("modalidad", $curso["modalidad"]) === "presencial" ? "checked" : "" ?>>
                <label class="btn btn-outline-primary" for="modalidad-presencial">Presencial</label>
                <input type="radio" class="btn-check" name="modalidad" id="modalidad-mixto" value="mixto" autocomplete="off" <?= cleanOld("modalidad", $curso["modalidad"]) === "mixto" ? "checked" : "" ?>>
                <label class="btn btn-outline-primary" for="modalidad-mixto">Mixto</label>
                <input type="radio" class="btn-check" name="modalidad" id="modalidad-virtual" value="virtual" autocomplete="off" <?= cleanOld("modalidad", $curso["modalidad"]) === "virtual" ? "checked" : "" ?>>
        ...
        <div class="col mb-3">
            ...
            <input type="date" class="form-control <?= isValidInput($errors, "fechaInicial") ?>" id="fecha-inicial" name="fecha-inicial" value="<?= cleanOld("fechaInicial", $curso["fecha_inicio"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['fechaInicial'] ?>
            </div>
        </div>
        ...
        <div class="col mb-3">
            ...
            <input type="date" class="form-control <?= isValidInput($errors, "fechaFinal") ?>" id="fecha-final" name="fecha-final" value="<?= cleanOld("fechaFinal", $curso["fecha_final"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['fechaFinal'] ?>
            </div>
        </div>
        ...
        <div class="col mb-3">
            ...
            <input type="number" class="form-control <?= isValidInput($errors, "horasTotal") ?>" id="horas-total" name="horas-total" value="<?= cleanOld("horasTotal", $curso["horas_total"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['horasTotal'] ?>
            </div>
        </div>
        <div id="horas-presenciales-container" class="col mb-3">
            ...
            <input type="number" class="form-control <?= isValidInput($errors, "horasPresenciales") ?>" id="horas-presenciales" name="horas-presenciales" value="<?= cleanOld("horasPresenciales", $curso["horas_presenciales"]) ?>">
            <div class="invalid-feedback">
        ...
        <div id="aula-container" class="col-12 mb-3">
            ...
            <input type="text" class="form-control <?= isValidInput($errors, "aula") ?>" id="aula" name="aula" value="<?= cleanOld("aula", $curso["aula"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['aula'] ?>
            </div>
        </div>
        ...
        <div id="dias-container" class="btn-group mb-3" role="group" aria-label="Basic checkbox toggle button group" id="dias">
            <?php foreach ($days as $day => $dayName): ?>
                <input type="checkbox" class="btn-check" <?= in_array($day, old("dias", $curso["dias"])) ? "checked" : "" ?> id="<?= $day ?>" name="dias[]" value="<?= $day ?>" autocomplete="off">
        ...
        <div id="hora-inicial-container" class="col mb-3">
            ...
            <input type="time" class="form-control <?= isValidInput($errors, "horaInicial") ?>" id="hora-inicial" name="hora-inicial" value="<?= cleanOld("horaInicial", $curso["hora_inicial"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['horaInicial'] ?>
            </div>
        </div>
        ...
        <div id="hora-final-container" class="col mb-3">
            ...
            <input type="time" class="form-control <?= isValidInput($errors, "horaFinal") ?>" id="hora-final" name="hora-final" value="<?= cleanOld("horaFinal", $curso["hora_final"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['horaFinal'] ?>
            </div>
        </div>
        ...
        <div id="limite-container" class="col-12 mb-3">
            ...
            <input type="number" class="form-control <?= isValidInput($errors, "limite") ?>" id="limite" name="limite" value="<?= cleanOld("limite", $curso["limite"]) ?>">
            <div class="invalid-feedback">
                <?= $errors['limite'] ?>
            </div>
        </div>
        <div id="externo-switch">
            <div class="form-check mb-2">
                ...
                <input class="form-check-input <?= isValidInput($errors, "externo") ?>" type="checkbox" role="switch" name="externo" id="externo" value="1" <?= cleanOld("externo", $curso["externo"]) === "1" ? "checked" : "" ?>>
                <div class="invalid-feedback">
                    <?= $errors['externo'] ?>
                </div>
            </div>
