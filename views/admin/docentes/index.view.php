...
<main role="main" class="container py-5" style="margin-top: 56px">
                ...
                <div class="col-md-6">
                    <div class="input-group flex-nowrap">
                        ...
                        <input
                            ...
                            aria-label="BuscarDocente"
                            aria-describedby="buscar-docente"
    ...
    <div class="row row-cols-auto justify-content-between py-3 px-2">
        <?php if ($paramsActive): ?>
            <a class="col" href="<?= $archivados ? "?archivados=true" : "/admin/docentes" ?>">
        ...
        <span class="col text-secondary text-end fst-italic">
            <?= $pagination["totalItems"] ?> <?= $pagination["totalItems"] == 1 ? "docente encontrado" : "docentes encontrados" ?>
        </span>
    </div>
    <?php if (empty($allDocentes)): ?>
            ...
            <table class="table table-striped table-hover align-middle">
                ...
                <tbody>
                    <?php foreach ($allDocentes as $docente) : ?>
                        <?php
                        $id = htmlspecialchars($docente['id']);
                        $username = htmlspecialchars($docente['username']);
                        $nombre = htmlspecialchars($docente['nombre']);
                        $esInstructor = htmlspecialchars($docente['es_instructor']);
                        $horasBase = htmlspecialchars($docente['horas_base']);
                        $email = htmlspecialchars($docente['email']);
                        ?>
                        <tr>
                            <td><?= $username ?></td>
                            <td><?= $nombre ?></td>
                            <td><span class="badge rounded-pill text-bg-primary"><?= $esInstructor ?></span></td>
                            <td><span class="badge rounded-pill text-bg-<?= $horasBase == 0 ? "danger" : "secondary" ?>"><?= $horasBase == 0 ? "Por horas" : $horasBase ?></span></td>
                            <td><?= $email ?></td>
                            <td class="text-end">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <?php if (!$archivados): ?>
                                            <li>
                                                <a class="dropdown-item" href="/admin/docentes/editar?id=<?= $id ?>">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="archiveModalLabel<?= $id ?>"><?= $archivados ? "Desarchivar" : "Archivar" ?> Docente</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <p>¿Seguro de que desea <?= $archivados ? "desarchivar" : "archivar" ?> este docente?</p>
                                        </div>
    ...
    <div class="d-flex flex-column align-items-center gap-3 pt-3">
        <span class="text-center text-secondary">¿No encuentras el docente que buscas?</span>
        <a href="<?= $archivados ? "/admin/docentes" : "?archivados=true" ?>" type="btn" class="btn btn-outline-secondary">
            <?= $archivados ? "Ver docentes activos" : "Ver docentes archivados" ?>
        </a>
    </div>
</main>
