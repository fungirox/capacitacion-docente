...
class DocenteForm extends FormTemplate {
    ...
    protected function validateForm() {
        ...
        if (!Validator::bit($this->attributes["baseHoras"])) {
            $this->errors["baseHoras"] = "Favor de seleccionar si el docente es de base o por horas.";
        }

        if ($this->attributes["baseHoras"] == 1) {
            if (!Validator::numeric($this->attributes["horasBase"])) {
                $this->errors["horasBase"] = "Favor de introducir un número de horas válido.";
            }
        }

        if (!Validator::bit($this->attributes["docenteInstructor"])) {
            $this->errors["docenteInstructor"] = "Favor de seleccionar si el docente es instructor o no.";
        }

        if ($this->attributes["docenteInstructor"] == 1) {
